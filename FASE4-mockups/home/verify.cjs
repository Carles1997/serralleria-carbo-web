...
(async () => {
  ...
  for (const [name, width, height] of [['desktop', 1440, 900], ['tablet', 768, 900], ['mobile', 390, 844], ['small-mobile', 320, 700]]) {
    ...
    const result = await page.evaluate(() => ({
      ...
      heroColumns: getComputedStyle(document.querySelector('.choice-grid')).gridTemplateColumns,
      fontLoaded: document.fonts.check('700 32px "Source Sans 3"'),
      imagesLoaded: [...document.querySelectorAll('main img')].every(img => img.complete && img.naturalWidth > 0),
      visiblePaths: [...document.querySelectorAll('.choice-card')].every(el => el.getBoundingClientRect().height > 0),
      industrialTitleFits: (() => { const el = document.querySelector('.industrial-link strong'); const card = el.closest('.industrial-link'); const range = document.createRange(); range.selectNodeContents(el); return [...range.getClientRects()].every(rect => rect.right <= card.getBoundingClientRect().right - 8); })(),
  ...
  await touchPage.locator('.path-shortcuts a').last().tap();
  const touchShortcut = new URL(touchPage.url()).hash === '#hero-industrial';
  await touchPage.locator('.choice-industrial').tap();
  const touchPath = new URL(touchPage.url()).hash === '#industrial';
  await touchPage.close();

  console.log(JSON.stringify({ results, before, afterHover, afterFocus, reducedMotion, menuOpened, menuClosed, touchShortcut, touchPath, errors }, null, 2));
  await browser.close();
  if (results.some(item => !item.noHorizontalOverflow || !item.fontLoaded || !item.imagesLoaded || !item.visiblePaths || !item.industrialTitleFits) || !menuOpened || !menuClosed || !touchShortcut || !touchPath || errors.length) process.exitCode = 1;
